"""
This module provides the main GUI frontend code for the portfolio backtesting tool
"""
...
from potion.streamlitapp.multibackt.ma_frontend_helper_functions import (
    initialize_multi_asset_backtester_session_state, load_multi_asset_settings_panel,
    load_multi_asset_results_panel)
...
def body():
    ...
    None
    """
    initialize_multi_asset_backtester_session_state()

    # print('Loading multi asset backtester panels')
    st.title('Pool Backtester')
    st.image(im)
    with st.container():

        # Get the inputs from the user
        batch_number = load_multi_asset_settings_panel()

        # Display the results after the user pressed the start button
        load_multi_asset_results_panel(batch_number)


if __name__ == '__main__':

    parser = argparse.ArgumentParser(prog='ma_frontend', usage='%(prog)s [options]')
    parser.add_argument('--multi', action='store_true',
                        help='Launches the Multi Asset Backtester tab in multi '
